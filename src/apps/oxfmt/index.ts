...
/**
 * A manifest passed as the first argument is accepted and discarded.
 *
 * `svelte` was briefly gated on the consuming project's `package.json`, so `defineConfig` took it
 * first and `datamitsu config reconcile` generated `defineConfig(packageJSON)`. Those files exist
 * in repositories that reconciled during that window, and without this they would be read as an
 * overrides object — spreading `name`, `version` and `dependencies` into the oxfmt config. A
 * manifest and an oxfmt config share no key in the pinned schema, so ordinary calls are told apart
 * reliably. It is still a heuristic at the edges, in both directions: an overrides object that also
 * carries a `name` loses its settings, and a manifest holding only `private` and `scripts` is
 * spread into the config. Neither shape is generated by anything here.
 *
 * Removable once no consuming repository still generates that call. Added 2026-09-23.
 */
const isPackageJson = (value: object): value is PackageJson =>
  "name" in value ||
  "version" in value ||
  "dependencies" in value ||
  "devDependencies" in value ||
  "peerDependencies" in value ||
  "optionalDependencies" in value;
...
export const defineConfig = (
  config?: ((base: OxfmtConfig) => OxfmtConfig) | OxfmtConfig | PackageJson,
  legacyOverrides?: OxfmtConfig,
): OxfmtConfig => {
  if (typeof config === "function") {
    return defineOxfmtConfig(config(baseConfig));
  }

  const overrides = config && isPackageJson(config) ? legacyOverrides : (config as OxfmtConfig);

  return defineOxfmtConfig(mergeConfig(baseConfig, overrides));
};
