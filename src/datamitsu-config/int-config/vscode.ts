...
export const vscodeExtensions: (context: config.ConfigContext) => string = (context) => {
...
const parseFileNestingPatterns = (
  value: Record<string, string> | undefined,
): Record<string, string[]> => {
  return Object.entries(value ?? {}).reduce<Record<string, string[]>>((acc, [key, el]) => {
...
const stringifyFileNestingPatterns = (value: Record<string, string[]>): Record<string, string> => {
  return Object.entries(value).reduce<Record<string, string>>((acc, [key, el]) => {
    acc[key] = el.join(", ");
...
export const vscodeSettings: (context: config.ConfigContext) => string = (context) => {
  const config = safeJsonParse(context.originalContent);
