...
export interface ConfigShowOutput {
  init: Record<string, ConfigInitData>;
}

export interface InitConfigDocInfo {
  deleteOnly: boolean;
  linkTarget: string;
  name: string;
  otherFileNameList: string[];
...
export function extractInitConfigsInfo(config: ConfigShowOutput): InitConfigDocInfo[] {
  return Object.entries(config.init)
    .map(([name, initConfig]) => ({
...
export function generateInitConfigsMarkdown(configs: InitConfigDocInfo[]): string {
  const lines = [
    "# Init Configurations",
    "",
    ...
    "",
    `This configuration includes **${configs.length} init configs**.`,
    "",
...
export async function main(): Promise<void> {
  ...
  console.log(`Generated ${outputPath} with ${configs.length} init configs`);
}

export function parseConfigJson(jsonStr: string): ConfigShowOutput {
  const data: unknown = JSON.parse(jsonStr);
  if (typeof data !== "object" || data === null || !("init" in data)) {
    throw new Error("Invalid config output: missing 'init' field");
  }
...
function generateInitConfigsTable(configs: InitConfigDocInfo[]): string {
