...
- `task validate:rule-inventory` — fails when the live rule set has drifted from the committed one, printing added / removed / re-severitied rules. Runs at the end of `task build` and in pre-commit, so a drifted rule set cannot be packaged.
- `task rules:inventory` — regenerates the file, i.e. accepts the current rule set as reviewed. This is the one step `task refresh` deliberately leaves to you: run it _after_ deciding what the new rules should be, not to make the build pass.
...
`task refresh` is the normal command. It regenerates everything derived from the sources in this repo and then checks the result — build, docs, and the guards. Reach past it only when you know which single task you want; `task --list` shows them all.
    ...
    task build                produce the package; ends by regenerating .oxlintrc.json and gating on the rule inventory
    task validate             validate:blocklist + validate:rule-inventory
